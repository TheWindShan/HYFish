...
// 公共宏定义
#define KIND_ID						507								// 游戏 I D

#define GAME_PLAYER					6								// 游戏人数
#define GAME_NAME					TEXT("红宇捕鱼")				// 游戏名字
...
struct GameScene
{
	...
	int					nFishMultiple[FISH_ALL_COUNT][2];		// 鱼倍数

	DWORD				nPlayPropIndex[GAME_PLAYER];			// 玩家道具信息
	DWORD				nPropConsume[PROP_COUNT_MAX];			// 道具消耗
	DWORD				nPropCount[PROP_COUNT_MAX];				// 拥有数量
	SCORE               lBulletConsume[GAME_PLAYER];			// 子弹消耗
	SCORE               lPropAcquire[GAME_PLAYER];				// 道具消耗
	int                 lPlayFishCount[GAME_PLAYER][FISH_ALL_COUNT];	// 玩家捕获鱼数
	int                 llPlayBulletCount[GAME_PLAYER][BULLET_MAX];	// 开炮数量
	int                 llPlayBulletMoney[GAME_PLAYER][BULLET_MAX];	// 开炮金币
	TCHAR				szBrowseUrl[256];						// 充值地址
	WORD				wCanAcceptNumber;						// 可接受数量

	int					nMultipleValue[MULTIPLE_MAX_INDEX];		// 房间倍数
	int					nMultipleIndex;							// 当前倍数
	bool				bCanSetMultiple;						// 可以设置倍数
...
struct CMD_S_FishCreate
{
	// 基本信息
	int					nFishIndex;				// 鱼索引
	DWORD				unCreateTime;			// 创建时间
	DWORD				unCreateDelay;			// 创建延迟
	WORD				wHitChair;				// 击杀位置
	BYTE				nFishType;				// 鱼种类
	bool				bSpecial;				// 特殊鱼
	bool				bKiller;				// 杀手鱼
...
enum
{
	...
	SUB_C_FIRE,										// 开火
	SUB_C_BUY_BULLET,								// 更换子弹
	SUB_C_DELAY,									// 延迟
	SUB_C_USE_PROP,									// 道具
	SUB_C_BUY_PROP,									// 购买道具
	SUB_C_SELECTED_PROP,							// 选中道具
	SUB_C_BEGIN_LASER,								// 准备激光
	SUB_C_LASER,									// 激光
	SUB_C_FISH_BOMB,								// 炸弹爆炸
	SUB_C_SPEECH,									// 语音消息
	SUB_C_MULTIPLE,									// 倍数消息
};
