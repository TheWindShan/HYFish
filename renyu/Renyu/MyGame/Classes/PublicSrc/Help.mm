...
void getCurrentVersion(WORD kindID)
{
    ...
    prequest->setResponseCallback([=](cocos2d::network::HttpClient *sender, cocos2d::network::HttpResponse *response){
        ...
        if (response == NULL)
        {
            ...
            if (HallDataMgr::getInstance()->_versionCheckcallback) {
        ...
        if ( !DICTOOL->checkObjectExist_json(val, "Version"))
        {
        ...
        std::string _version = DICTOOL->getStringValue_json(val, "Version");
        std::string updateStr = DICTOOL->getStringValue_json(val, "DownloadUrl");
        const char *URL = updateStr.c_str();
        
        JniMethodInfo minfo;
        if (JniHelper::getStaticMethodInfo(minfo, "org/cocos2dx/cpp/AppActivity", "UpdateConfirm", "(Ljava/lang/String;Ljava/lang/String;)V"))
        {
            jstring jVersion = minfo.env->NewStringUTF( _version.c_str());
            jstring jURL = minfo.env->NewStringUTF(URL);
            minfo.env -> CallStaticVoidMethod(minfo.classID,minfo.methodID,jVersion,jURL);
        }
#endif
    ...
    postData += "&";
    postData += "PlatfromID=";
    postData += VersionAndroidPlatform;
#endif
    postData += "&";
    postData += __String::createWithFormat("ServerTime=%lld",time)->getCString();
    log("请求参数---------------------------------------%s",postData.c_str());
...
extern "C"
{
    void Java_org_cocos2dx_cpp_AppActivity_UpdateConfirm(JNIEnv *env,jobject thiz)
    {
        
        auto player = static_cast<ModeLayer *>(HallDataMgr::getInstance()->AddpopLayer("提示", "发现新版本,是否前往更新", Type_Ensure_Cancel));
        player->setEnsureCallback([=](){
            JniMethodInfo minfo;
            if (JniHelper::getStaticMethodInfo(minfo, "org/cocos2dx/cpp/AppActivity", "SureUpdate", "()V"))
            {
