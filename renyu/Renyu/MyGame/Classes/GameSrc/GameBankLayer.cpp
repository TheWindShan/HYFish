...
bool GameBankLayer::init()
{
    ...
    pbg->setPreferredSize(cocos2d::Size(760, 520));
    ...
    ptitle->setPosition(Vec2(pbg->getContentSize().width/2, pbg->getContentSize().height-50));
    pbg->addChild(ptitle);
...
void GameBankLayer::accessinit()
{
    ...
    //userscore->setDimensions(150, 30);
    ...
    if (puser) {
        scorestring = __String::createWithFormat("%lld", puser->m_date.lScore)->getCString();
        if (scorestring.length()>12) {
            scorestring = scorestring.substr(0,10);
            scorestring.append("..");
        }
    ...
    //insuerscore->setDimensions(150, 30);
...
void GameBankLayer::updateScore()
{
    ...
    if (accesslayout) {
        ...
        auto puser = HallDataMgr::getInstance()->m_UserList.at(HallDataMgr::getInstance()->m_dwUserID);
        if (puser) {
            std::string scorestring = __String::createWithFormat("%lld", puser->m_date.lScore)->getCString();
            if (scorestring.length()>12) {
                scorestring = scorestring.substr(0,10);
                scorestring.append("..");
            }
...
void GameBankLayer::sendInsureEnable(const std::string &pass)
{
    ...
    UTF8Str_To_UTF16Str(pass, insuer.szInsurePass);
...
void GameBankLayer::sendInsureInfo()
{
    ...
    else
    {
        ...
        UTF8Str_To_UTF16Str(pass, info.szInsurePass);
    }
