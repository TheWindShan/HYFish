...
bool DeskLayer::init()
{
    ...
    auto pbegin = Button::create("room_res/bt_quick_begin.png");
    pbegin->setTag(Tag_BT_Begin);
    pbegin->addTouchEventListener(CC_CALLBACK_2(DeskLayer::touchEvent, this));
    pbegin->setPosition(Vec2(viewsize.width/2-420, viewsize.height/2+70));
    this->addChild(pbegin);
    
    auto pencrypt = Button::create("room_res/bt_desk_encrypt.png");
    pencrypt->setTag(Tag_BT_Encrypt);
    pencrypt->addTouchEventListener(CC_CALLBACK_2(DeskLayer::touchEvent, this));
    pencrypt->setPosition(Vec2(viewsize.width/2-420, viewsize.height/2-70));
    this->addChild(pencrypt);
    
    
    m_pageIndex = 0;
    m_pageView = PageView::create();
    m_pageView->setContentSize(cocos2d::Size(770, 390));
    m_pageView->setAnchorPoint(Vec2(0.5f, 0.5f));
    m_pageView->setTag(Tag_PageView);
    m_pageView->setPosition(Vec2(viewsize.width/2+40, viewsize.height/2+8));
    ...
    int judgevalue = HallDataMgr::getInstance()->m_TableCount%PAGE_TABLE_COUNT;
    int pagenum = HallDataMgr::getInstance()->m_TableCount/PAGE_TABLE_COUNT;
    pagenum = judgevalue?(pagenum+1):pagenum;
    for (int index=0; index<pagenum; ++index)
    {
        auto pdot = index==0?ImageView::create("room_res/im_page_0.png"):ImageView::create("room_res/im_page_1.png");
        pdot->setPosition(Vec2(viewsize.width/2-305+36*index, 120));
        pdot->setTag(Tag_Dot+index);
        this->addChild(pdot);
        ...
        for (int deskid=0; deskid<3; ++deskid) {
            int number = index*3+deskid;
            auto pdesk = DeskCell::create("room_res/desk_bg.png", number);
            pdesk->setTag(deskid);
            pdesk->setPosition(Vec2(playout->getContentSize().width/2, playout->getContentSize().height-pdesk->getContentSize().height*(deskid+0.5f)));
...
void DeskLayer::userinfoUpdate(float t)
{
    ...
    NetworkMgr::getInstance()->sendRequestUserInfo(INVALID_TABLE, m_pageIndex*PAGE_TABLE_COUNT);
}
        ...
        case Widget::TouchEventType::ENDED:
        {
            ...
            switch (ntag) {
                case Tag_BT_Begin:
                {
                    ...
                    NetworkMgr::getInstance()->sendData(MDM_GR_USER, SUB_GR_USER_CHAIR_REQ, nullptr, 0);
                }
                    break;
                case Tag_BT_Encrypt:
                {
...
void DeskLayer::OnUserEnter(void *pUser)
{
    ...
    if (puserData->m_date.wTableID/PAGE_TABLE_COUNT != m_pageIndex) {
        return;
    }
...
void DeskLayer::OnUserStatus(void *pUser)
{
    ...
    if (puserData->m_date.cbUserStatus >= US_SIT) {
        auto playout = m_pageView->getPage(puserData->m_date.wTableID/PAGE_TABLE_COUNT);
        if (!playout) {
            return;
        }
        auto pcell = (DeskCell *)playout->getChildByTag(puserData->m_date.wTableID%PAGE_TABLE_COUNT);
        if (!pcell) {
            return;
        }
    ...
    else if (puserData->m_date.cbUserStatus == US_FREE)
    {
        ...
        auto playout = m_pageView->getPage(deskuser->m_tableID/PAGE_TABLE_COUNT);
        if (!playout) {
            return;
        }
        auto pcell = (DeskCell *)playout->getChildByTag(deskuser->m_tableID%PAGE_TABLE_COUNT);
        if (!pcell) {
            return;
        }
