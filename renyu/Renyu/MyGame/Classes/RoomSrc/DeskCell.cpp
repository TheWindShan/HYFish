...
{
    auto number = TextAtlas::create(__String::createWithFormat("%d",index+1)->getCString(), "room_res/num_desk.png", 34, 47, "0");
    number->setPosition(Vec2(60, this->getContentSize().height/2));
    ...
    for (int seatid=0; seatid<HallDataMgr::getInstance()->m_ChairCount; ++seatid) {
        auto seat = Button::create("room_res/desk_seat.png");
        seat->setPressedActionEnabled(false);
        seat->addTouchEventListener(CC_CALLBACK_2(DeskCell::touchEvent, this));
        seat->setPosition(Vec2(150+104*seatid, this->getContentSize().height/2+10));
...
void DeskCell::userEnter(UserData *pdata, WORD chairID)
{
    ...
    playout->setPosition(Vec2(150+104*chairID, this->getContentSize().height/2));
    playout->setTag(tag);
    this->addChild(playout);
    
    auto head = HeaderRequest::createwithFaceID(pdata->m_date.wFaceID, pdata->m_date.dwCustomID, pdata->m_date.dwUserID);
    head->setHeadSize(80);
    head->setPosition(Vec2(playout->getContentSize().width/2, playout->getContentSize().height-44));
    playout->addChild(head);
